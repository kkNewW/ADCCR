...
class COCODataset(Dataset):
    ...
    def _parse_data_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # extra fields for analysis / refinement
        data_dict["kpt_name"] = kpt_name[:5]
        data_dict["description"] = kpt_des[:5]
        data_dict["desc_mode"] = desc_mode_list[:5]
        data_dict["answer_text"] = caption[:5]
        data_dict["target_xy_norm"] = torch.tensor(target_xy_list[:5], dtype=torch.float32)
        data_dict["target_xy_224"] = torch.tensor(target_xy_224_list[:5], dtype=torch.float32)
