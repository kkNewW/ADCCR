...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # ===== keep extra fields for analysis / refinement =====
        # if "kpt_name" in instances[0]:
        #     batch["kpt_name"] = [instance["kpt_name"] for instance in instances]

        # if "description" in instances[0]:
        #     batch["description"] = [instance["description"] for instance in instances]

        # if "desc_mode" in instances[0]:
        #     batch["desc_mode"] = [instance["desc_mode"] for instance in instances]

        # if "answer_text" in instances[0]:
        #     batch["answer_text"] = [instance["answer_text"] for instance in instances]

        # if "target_xy_norm" in instances[0]:
        #     batch["target_xy_norm"] = torch.stack(
        #         [instance["target_xy_norm"] for instance in instances], dim=0
        #     )

        # if "target_xy_224" in instances[0]:
        #     batch["target_xy_224"] = torch.stack(
        #         [instance["target_xy_224"] for instance in instances], dim=0
        #     )
